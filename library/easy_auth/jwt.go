...
func NewJWT() *JWT {
	return &JWT{
		SigningKey: []byte(easy_config.Viper.GetString("jwt.signingKey")),
	}
}
...
func (j *JWT) CreateClaims(baseClaims BaseClaims) MyClaims {
	claims := MyClaims{
		...
		StandardClaims: jwt.StandardClaims{
			...
			ExpiresAt: time.Now().Unix() + int64(easy_config.Viper.GetInt("jwt.expiresTime")), // 过期时间
			Issuer:    easy_config.Viper.GetString("jwt.issuer"),                              // 签名的发行者
		},
	}
	return claims
}
